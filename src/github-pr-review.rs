...
async fn handler(
    ...
    match pulls.get_diff(pull_number).await {
        Ok(diff) => {
            resp.push_str("\n\n");

            log::debug!("Sending patch to OpenAI:{}", pull_number);
            let co = ChatOptions {
                model: MODEL,
                restart: true,
            ...
            match openai.chat_completion(&chat_id, &question, &co).await {
                Ok(r) => {
                    ...
                    resp.push_str("\n\n");
                    log::debug!("Received OpenAI resp for patch:{}", pull_number);
                }
                Err(_e) => {
                    log::error!("OpenAI returns error for patch review for {}", pull_number);
                }
            }
        },
        Err(_error) => {
            log::error!("Cannot get diff content");
        }
    }
